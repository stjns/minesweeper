...
})({"s1TyZ":[function(require,module,exports) {
...
var _gameBoard = require("./models/GameBoard");
let boardDiv = document.getElementById("game-board");
const gameBoard = new (0, _gameBoard.GameBoard)(10, 10);
boardDiv.appendChild(gameBoard.generateInitialBoard());

},{"./models/GameBoard":"5seMF"}],"5seMF":[function(require,module,exports) {
...
class GameBoard {
    ...
    populateBoard() {
        ...
        for(let y = 0; y < this.boardWidth; y++)for(let x = 0; x < this.boardHeight; x++){
            const newCell = new (0, _gameCell.GameCell)(this.isMined(), ()=>this.propagateEmptyCell(x, y));
            this.cells[x][y] = newCell;
        }
        //then calculate the mined neighbors
        for(let y = 0; y < this.boardWidth; y++){
            for(let x = 0; x < this.boardHeight; x++)if (!this.cells[x][y].mined) {
                let count = 0;
    ...
    propagateEmptyCell(x, y) {
        if (this.cells[x][y].isEmpty()) for(let x2 = x - 1; x2 < x + 2; x2++){
            for(let y2 = y - 1; y2 < y + 2; y2++)if (x2 >= 0 && x2 < this.boardWidth && y2 >= 0 && y2 < this.boardHeight && !this.cells[x2][y2].isRevealed()) {
                ...
                if (this.cells[x2][y2].isEmpty()) this.propagateEmptyCell(x2, y2);
            }
        }
    }
    generateInitialBoard() {
        ...
        for(let y = 0; y < this.boardWidth; y++){
            ...
            for(let x = 0; x < this.boardHeight; x++)newCellRow.appendChild(this.cells[x][y].el);
...
},{"./GameCell":"dPaaS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"dPaaS":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "GameCell", ()=>GameCell);
class GameCell {
    constructor(mined = false, emptyCellCallback){
        this.mined = false;
    ...
    reveal() {
        if (!this.flagged && !this.revealed) {
            ...
            if (this.mined) {
                ...
                this.el.innerHTML = "&#x1f4a3;";
            } else if (this.mineNeighbors > 0) this.el.textContent = this.numNearbyMines().toString();
            else this.emptyCellCallback();
        }
    }
    onCellClick() {
    ...
    onFlagToggle() {
        if (!this.isRevealed()) {
            if (!this.isFlagged()) this.el.innerHTML = "&#x26f3;";
...
},{}]},["s1TyZ","7ZSs6"], "7ZSs6", "parcelRequired5d1")
